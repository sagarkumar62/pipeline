...
logger = setup_logger("run_phase15_companies")
...
async def main():
    logger.info("=== STARTING PHASE 15: COMPANIES MODULE INGESTION RUN ===")

    out_dir = Path("data/working/companies")
    out_dir.mkdir(parents=True, exist_ok=True)

    # 28 Protected files pre-check verification
    protected_files = [
    ...
    # 1. Discovery
    adapter = CompanyAdapter(max_pages_per_query=2, per_page=30)
    raw_candidates = []
    ...
    raw_path = out_dir / "companies_raw.jsonl"
    ...
    with open(out_dir / "companies_qualified.jsonl", "w", encoding="utf-8") as f:
        for r in qualified_records:
            f.write(json.dumps(r) + "\n")

    with open(out_dir / "companies_rejected.jsonl", "w", encoding="utf-8") as f:
        for r in rejected_records:
            f.write(json.dumps(r) + "\n")

    with open(out_dir / "companies_review.jsonl", "w", encoding="utf-8") as f:
    ...
    logger.info(f"Company Deduplication Complete: Unique={len(deduped_records)}, Duplicates={duplicates_count}")
    ...
    final_path = out_dir / "companies_final.json"
    with open(final_path, "w", encoding="utf-8") as f:
        json.dump(final_records, f, indent=2)

    output_hashes = {
        "companies_raw.jsonl": get_file_hash(str(out_dir / "companies_raw.jsonl")),
        "companies_qualified.jsonl": get_file_hash(str(out_dir / "companies_qualified.jsonl")),
        "companies_rejected.jsonl": get_file_hash(str(out_dir / "companies_rejected.jsonl")),
        "companies_review.jsonl": get_file_hash(str(out_dir / "companies_review.jsonl")),
        "companies_final.json": get_file_hash(str(final_path))
    }
    ...
    manifest = {
        ...
        "source_counts": {
            "GitHub Organizations API": len(raw_candidates)
        },
        ...
        "output_files": {
            "raw": str(out_dir / "companies_raw.jsonl"),
            "qualified": str(out_dir / "companies_qualified.jsonl"),
            "rejected": str(out_dir / "companies_rejected.jsonl"),
            "review": str(out_dir / "companies_review.jsonl"),
    ...
    manifest_path = out_dir / "companies_manifest.json"
    ...
    audit_data = {
        ...
        "sources_used": ["GitHub Organizations API"],
        "sources_unaccessible_or_omitted": [
            "TAAFT AI Companies (No public API available)",
            "Crunchbase (Paid API / Credential restricted)",
            "Tracxn (Paid API / Credential restricted)"
        ],
        ...
        "company_types_breakdown": {},
        "status_breakdown": {
            "ACTIVE": len(final_records)
        },
    ...
    for r in final_records:
        ...
        audit_data["company_types_breakdown"][ct] = audit_data["company_types_breakdown"].get(ct, 0) + 1

    audit_path = Path("data/working/phase15_companies_audit.json")
    ...
    logger.info(f"=== PHASE 15 COMPANY INGESTION COMPLETE. Final count: {len(final_records)} records saved to {final_path} ===")
