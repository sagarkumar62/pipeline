"""
Phase 4 Groq API Provider (Fallback 1)

Groq API implementation (Llama-3) supporting evidence-based description generation,
...
class GroqProvider(BaseLLMProvider):
    """Groq API Provider (Llama-3 model)."""
    ...
    async def generate_description(self, evidence: EvidencePackage) -> LLMResult:
        if not self.is_available:
            return LLMResult(
                ...
                status="FALLBACK",
                error_message="Groq API key not configured or auth failed",
            )
            ...
            try:
                async with httpx.AsyncClient(timeout=15.0) as client:
                    ...
                    if res.status_code in (401, 403):
                        ...
                        logger.error(f"Groq API Auth Failure HTTP {res.status_code}. Marking provider unavailable.")
                        return LLMResult(
                            provider_name=self.provider_name,
                            status="AUTH_ERROR",
                        ...
                        logger.warning(f"Groq API 429 Rate Limited (attempt {attempt}/{retries}).")
                        if attempt < retries:
                            retry_after = float(res.headers.get("Retry-After", 5.0 * attempt))
                        ...
                        try:
                            ...
                            logger.warning(f"Groq API JSON parse error: {parse_err}. Raw text: {raw_text[:100]}")
                            return LLMResult(
                                provider_name=self.provider_name,
                                status="MALFORMED",
                        ...
                        logger.warning(f"Groq API HTTP {res.status_code}: {res.text}")
                        if attempt <= retries:
                ...
                latency = (time.time() - start_time) * 1000.0
                logger.warning(f"Groq API Timeout (attempt {attempt}).")
                if attempt <= retries:
                    await asyncio.sleep(1.0 * attempt)
                    continue
                return LLMResult(
