...
logger = setup_logger("robot_resolver")


class RobotDeduplicationResolver:
    """
    Deterministic deduplication and entity resolution engine for Robot entities.
    
    Enforces distinct identity rules:
    - Distinct robot generations (e.g., Atlas vs Atlas Electric) remain distinct canonical models.
    - Resolution requires exact manufacturer + robot model identity or exact URL matching.
    
    Resolution Priority:
    1. Exact Registry ID / Stable Entity ID
    2. Normalized Repository URL match
    3. Canonical Official Product URL match
    4. Manufacturer/Robot Name identity pair match
    """
    ...
    def __init__(self):
        ...
        self._mfg_name_map: Dict[str, str] = {}         # "manufacturer/robot_name" -> canonical_id
    ...
    def load_baseline(self, records: List[Dict[str, Any]]) -> int:
        """Pre-loads existing Robot records into deduplication indexes."""
        count = 0
        for r in records:
        ...
        logger.info(f"Loaded {count} baseline Robot records into resolver indexes.")
        return count

    def resolve(self, record: Dict[str, Any]) -> Tuple[Dict[str, Any], bool]:
        """
        Resolves a candidate Robot record against existing indexed Robot entities.
        ...
        if rec_id and rec_id in self._seen_ids:
            ...
            logger.info(f"Duplicate Robot found by exact ID: '{rec_id}'")
            return record, True
        ...
        if norm_url and norm_url in self._repo_url_map:
            ...
            record["match_confidence"] = 1.0
            record["is_baseline_match"] = canonical.get("is_baseline", False)
            logger.info(f"Duplicate Robot found by repository URL: '{norm_url}'")
            return record, True
        ...
        if norm_off and norm_off in self._official_url_map:
            ...
            record["match_confidence"] = 1.0
            record["is_baseline_match"] = canonical.get("is_baseline", False)
            logger.info(f"Duplicate Robot found by official URL: '{norm_off}'")
            return record, True
        ...
        if mfg_name_key and mfg_name_key in self._mfg_name_map:
            ...
            record["match_confidence"] = 1.0
            record["is_baseline_match"] = canonical.get("is_baseline", False)
            logger.info(f"Duplicate Robot found by manufacturer/name pair: '{mfg_name_key}'")
            return record, True
